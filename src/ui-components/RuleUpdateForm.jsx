...
export default function RuleUpdateForm(props) {
  const {
    id: idProp,
    rule: ruleModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = ruleRecord
      ? { ...initialValues, ...ruleRecord }
      : initialValues;
  ...
  const [ruleRecord, setRuleRecord] = React.useState(ruleModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Rule, idProp)
        : ruleModelProp;
      setRuleRecord(record);
    };
    queryData();
  }, [idProp, ruleModelProp]);
  React.useEffect(resetStateValues, [ruleRecord]);
  const validations = {
    title: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Rule.copyOf(ruleRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "RuleUpdateForm")}
      {...rest}
    >
      <TextField
        label="Title"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || ruleModelProp)}
